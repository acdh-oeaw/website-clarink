...
function createProjectsCollection<TLocale extends IntlLocale>(locale: TLocale) {
	const language = getIntlLanguage(locale);
	const collection = withI18nPrefix("projects", language);
	...
	return createCollection({
		name: collection,
		directory: `./content/${language}/projects/`,
		include: ["*/index.mdx"],
		read(item) {
...
export const projects = {
	de: createProjectsCollection("de-AT"),
	en: createProjectsCollection("en-GB"),
};
