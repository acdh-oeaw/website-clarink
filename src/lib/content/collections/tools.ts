...
function createToolsCollection<TLocale extends IntlLocale>(locale: TLocale) {
	const language = getIntlLanguage(locale);
	const collection = withI18nPrefix("tools", language);
	...
	return createCollection({
		name: collection,
		directory: `./content/${language}/tools/`,
		include: ["*/index.mdx"],
		read(item) {
...
export const tools = {
	de: createToolsCollection("de-AT"),
	en: createToolsCollection("en-GB"),
};
